...
class PlaybackEngine:
    ...
    The engine owns playback state transitions only. It does not talk to music
    providers, and it does not alter API contracts. Instead it uses QueueService
    as the source of truth for queue ordering, which allows future playback
    providers to plug in behind this engine without changing the orchestration
    surface used by services or routes.
    """
    ...
    def pause(self, room_id: str) -> PlaybackSession:
        ...
        session.state = PlaybackState.PAUSED if session.current_song else PlaybackState.IDLE
    ...
    def _append_event(
        ...
        session.events.append(
            PlaybackEvent(
                ...
                metadata={"queue_position": session.queue_position},
            )
        )
